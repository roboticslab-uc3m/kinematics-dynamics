...
bool teo::TeoCollisionAvoidance::configure(yarp::os::ResourceFinder &rf) {

    const double defautTr = M_PI/180.0;
    std::string env = DEFAULT_ENV;
    double jmcMs = DEFAULT_TEO_COLLISION_AVOIDANCE_MS;
    std::string physics = DEFAULT_PHYSICS;
    int viewer = DEFAULT_VIEWER;

    if(rf.check("help")) {
        printf("TeoCollisionAvoidance options:\n");
    ...
    printf("TeoCollisionAvoidance using context: %s.\n",rf.getContext().c_str());
    printf("TeoCollisionAvoidance using env: %s, jmcMs: %f, physics: %s, viewer: %d.\n", env.c_str(), jmcMs, physics.c_str(), viewer);
    ...
    //Openrave Tolerance Test, no working (not implemented in openrave)
//    OpenRAVE::dReal tolerance;
//    tolerance=-100;
//    environmentPtr->GetCollisionChecker()->SetCollisionOptions(2);//OpenRAVE::CollisionOptions.Distance);
//    environmentPtr->GetCollisionChecker()->SetTolerance(tolerance); //OpenRAVE tolerance not used mb ask Juan
//    //std::cout<<environmentPtr->GetCollisionChecker()->GetCollisionOptions()<<std::endl;
//    //printf("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
    ...
    teoCollisionAvoidanceRateThread.setRate(jmcMs);
    teoCollisionAvoidanceRateThread.setEnvironmentPtr(environmentPtr);
    teoCollisionAvoidanceRateThread.setPtrVectorOfRobotPtr(&vectorOfRobotPtr);
    teoCollisionAvoidanceRateThread.setPtrVectorOfControlboardContainerPtr(&vectorOfControlboardContainerPtr);

    teoCollisionAvoidanceRateThread.setPtrVectorOfSensorPtrForCameras(&vectorOfSensorPtrForCameras);
    teoCollisionAvoidanceRateThread.setPtrVectorOfCameraSensorDataPtr(&vectorOfCameraSensorDataPtr);
    teoCollisionAvoidanceRateThread.setPtrVectorOfRgbPortPtr(&vectorOfRgbPortPtr);
    teoCollisionAvoidanceRateThread.setPtrVectorOfIntPortPtr(&vectorOfIntPortPtr);
    teoCollisionAvoidanceRateThread.setPtrVectorOfForce6DPortPtr(&vectorOfForce6DPortPtr);
    teoCollisionAvoidanceRateThread.setPtrVectorOfCameraWidth(&vectorOfCameraWidth);
    teoCollisionAvoidanceRateThread.setPtrVectorOfCameraHeight(&vectorOfCameraHeight);
    teoCollisionAvoidanceRateThread.setPtrVectorOfSensorPtrForLasers(&vectorOfSensorPtrForLasers);
    teoCollisionAvoidanceRateThread.setPtrVectorOfLaserSensorDataPtr(&vectorOfLaserSensorDataPtr);
    teoCollisionAvoidanceRateThread.setPtrVectorOfSensorPtrForForce6Ds(&vectorOfSensorPtrForForce6Ds);
    teoCollisionAvoidanceRateThread.setPtrVectorOfForce6DSensorDataPtr(&vectorOfForce6DSensorDataPtr);


    teoCollisionAvoidanceRateThread.start();
...
double teo::TeoCollisionAvoidance::getPeriod() {
    return 5.0;
}

// -----------------------------------------------------------------------------

bool teo::TeoCollisionAvoidance::updateModule() {
    CD_INFO("Alive...\n");
    return true;
}
...
bool teo::TeoCollisionAvoidance::close() {
    printf("[TeoCollisionAvoidance] begin: close(). Ask thread to stop...\n");
    teoCollisionAvoidanceRateThread.askToStop();
    printf("[TeoCollisionAvoidance] Done. Closing devices...\n");
    ...
    printf("[TeoCollisionAvoidance] Devices closed. Closing environment...\n");
    environmentPtr->Destroy(); // destroy
    yarp::os::Time::delay(0.4);
    printf("[TeoCollisionAvoidance] close() joining...\n");
    orThreads.join_all(); // wait for the viewer thread to exit
    printf("[TeoCollisionAvoidance] success: join_all ended.\n");
    printf("[TeoCollisionAvoidance] end: close().\n");
    return true;
}

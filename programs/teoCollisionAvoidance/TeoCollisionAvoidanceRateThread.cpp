...
#include "TeoCollisionAvoidanceRateThread.hpp"

// ------------------- RateThread Related ------------------------------------

bool teo::TeoCollisionAvoidanceRateThread::threadInit() {
    printf("[TeoCollisionAvoidanceRateThread] begin: threadInit()\n");
    jmcMs = this->getRate();
    ...
    printf("[TeoCollisionAvoidanceRateThread] end: threadInit()\n");
    return true;
}

// -----------------------------------------------------------------------------

void teo::TeoCollisionAvoidanceRateThread::run() {
    //printf("[TeoCollisionAvoidanceRateThread] run()\n");
    ...
    //Collision Checker
    {
        OpenRAVE::EnvironmentMutex::scoped_lock lock(environmentPtr->GetMutex()); // lock environment to prevent external changes
        //CCB.SetTolerance(tolerance);
        for(size_t i=0;i<ptrVectorOfRobotPtr->size();i++) {  // For each robot
            //Checking colisions after movement

            if (environmentPtr->CheckSelfCollision(ptrVectorOfRobotPtr->at(i))){
